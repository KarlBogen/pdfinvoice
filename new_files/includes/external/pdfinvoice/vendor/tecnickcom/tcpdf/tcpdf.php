...
// BOF Anpassung Karl 27.11.2025 PDF/A Fehler Parameter MODDATE fehlt
//					$out .= '<< /Type /EmbeddedFile'.$filter.' /Length '.strlen($stream).' /Params <</Size '.$rawsize.'>> >>';
					$out .= '<< /Type /EmbeddedFile'.$filter.' /Length '.strlen($stream).' /Params <</Size '.$rawsize.'>> /Params <</ModDate (D:'.@date('YmdHis').')>> >>';
// EOF Anpassung Karl 27.11.2025 PDF/A Fehler Parameter MODDATE fehlt
					$out .= ' stream'."\n".$stream."\n".'endstream';
					$out .= "\n".'endobj';
					$this->_out($out);
				}
